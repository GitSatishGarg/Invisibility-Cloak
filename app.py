...
while(cap.isOpened()):
    ...
    mask1 = cv2.morphologyEx(mask1, cv2.MORPH_OPEN,
                                np.ones((3, 3), np.uint8), iterations = 5) # Removing Noise
    mask1 = cv2.morphologyEx(mask1, cv2.MORPH_DILATE,
                                np.ones((3, 3), np.uint8), iterations = 1)

    mask2 = cv2.bitwise_not(mask1)  # Evryting except cloak

    res1 = cv2.bitwise_and(background, background, mask=mask1) # Used for segemntation of color
    res2 = cv2.bitwise_and(img, img, mask=mask2) # Used to substitute the cloak part
    final_output = cv2.addWeighted(res1, 1, res2, 1, 0)
    
    cv2.imshow('Ron Im Invisible!', final_output)
    k = cv2.waitKey(10)
    if k == 27:
        break
